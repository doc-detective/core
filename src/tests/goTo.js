...
async function goTo({ config, step, driver }) {
  ...
  try {
    ...
    // Wait for page to load with enhanced wait logic
    ...
    try {
      ...
      if (remainingTimeout <= 0) {
        throw new Error("Timeout exceeded after document ready");
      }

      // 2. Wait for network idle if enabled
      if (waitConditions.networkIdle && step.goTo.wait.networkIdleTime !== null) {
        try {
          await waitForNetworkIdle(driver, step.goTo.wait.networkIdleTime, remainingTimeout);
      ...
      const elapsedTime2 = Date.now() - waitStartTime;
      const remainingTimeout2 = waitTimeout - elapsedTime2;

      if (remainingTimeout2 <= 0) {
        throw new Error("Timeout exceeded after network idle check");
      }

      // 3. Wait for DOM stable if enabled
      if (waitConditions.domStable && step.goTo.wait.domIdleTime !== null) {
        try {
          await waitForDOMStable(driver, step.goTo.wait.domIdleTime, remainingTimeout2);
      ...
      if (waitConditions.elementFound && step.goTo.wait.find) {
        try {
          ...
          const selectorMsg = step.goTo.wait.find.selector ? `selector: "${step.goTo.wait.find.selector}"` : "";
          const textMsg = step.goTo.wait.find.elementText ? `text: "${step.goTo.wait.find.elementText}"` : "";
          const combinedMsg = [selectorMsg, textMsg].filter(m => m).join(", ");
...
async function waitForNetworkIdle(driver, idleTime, timeout) {
  ...
  const monitorState = await driver.execute(() => {
    return window.__docDetectiveNetworkMonitor || { lastRequestTime: Date.now(), requestCount: 0 };
  });
  
  if (Date.now() - monitorState.lastRequestTime >= 100 && monitorState.requestCount === 0) {
  ...
  while (true) {
    ...
    const monitorState = await driver.execute(() => {
      return window.__docDetectiveNetworkMonitor || { lastRequestTime: Date.now() };
    });
...
async function waitForDOMStable(driver, idleTime, timeout) {
  try {
    await driver.execute((idleMs, timeoutMs) => {
      return new Promise((resolve, reject) => {
        ...
        const checkInterval = setInterval(() => {
          ...
          if (idleFor >= idleMs) {
            ...
            clearInterval(checkInterval);
            observer.disconnect();
            reject(new Error(`Still mutating (${mutationCount} mutations in last ${idleMs}ms)`));
          }
        }, 100);
      });
    }, idleTime, timeout);
...
async function waitForElement(driver, findConfig, timeout) {
  ...
  if (selector && elementText) {
    ...
    while (true) {
      if (Date.now() - startTime > timeout) {
        throw new Error(`Element found but does not contain text: "${elementText}"`);
      }
